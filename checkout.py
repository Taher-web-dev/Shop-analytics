...
def detect_customer_face():
  ...
  while loop:
    ...
    encoding = face_recognition.face_encodings(resized_frame)
    ...
    for i, face in enumerate(faces):
      ...
      if key == ord('y'):
        ...
        matching = face_recognition.compare_faces(encoding, client_encoding,tolerance=0.6)
        ref = [r for r,val in enumerate(matching) if val==True][0]
        id = data_base.loc[ref,'id']
        try:
          names = data_base.loc[data_base['id']==id,"name"].values.tolist()
          names = [val for val in names if val != 'unknown']
          name = names[0]
        except:
        ...
        data_base.loc[data_base['id'] == id,'name'] = name
